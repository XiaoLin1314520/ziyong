...
UI.AddSliderFloat(["Misc.", "View", "Camera"], "Aspect ratio", 0.0, 5.0);

function aspectratio() {

    var value = UI.GetValue(["Misc.", "View", "Camera", "Aspect ratio"]).toString();
...

{
    ...
    const fov = UI.GetValue(["Misc.", "View", "Camera", "Field of view"]);

    const fovDifference = 90 - fov;

    const fovScope = 90 + fovDifference;

    const thirdperson = UI.GetValue(["Misc.", "Keys", "Keys", "Thirdperson"]);
    ...
    if (scoped) {
        ...
        if (!thirdperson) {
            ...
            UI.SetValue(["Misc.", "View", "Camera", "Override FOV while scoped"], 0);

        }

        else {

            UI.SetValue(["Misc.", "View", "Camera", "Override FOV while scoped"], UI.GetValue(["Config", "Custom scope", "Custom scope", "Override FOV while scoped"]) ? 0 : 1);
